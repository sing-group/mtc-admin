...
export default class GameResultGetManyRequestAdapter {
  ...
  adapt(builder, params) {
    ...
    if (params.target === "assigned-session") {
      check.assert.function(builder.listByAssignedGamesSession, "builder should have a listByPatient method");

      return builder.listByAssignedGamesSession(params.id, QueryOptions.fromAORParams(params));
    } else {
